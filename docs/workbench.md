...
`workbench producer-artifact artifact.json` and `workbench issue-execution contract.json` retain the existing submission v1 intake profile. The [candidate contract v2](../contracts/v2/README.md) adds pre-dispatch trial tickets, independent Git base registration, exact patch reconstruction, and executable candidate assessment. Producer intake remains `awaiting_independent_evaluation`; behavior and final-state observations determine acceptance.

Open **Candidate studies** for reserved software cohorts. Choose a study to compare initial outcomes, filter assisted corrections separately, inspect failed checks and receipt identities, and download a portable decision bundle. Resource-policy previews preserve original verdicts and require independent measurements for configured limits. Curator access is required for exports, policy previews and expected-versus-observed investigation; producer roles cannot retrieve reference suites.
