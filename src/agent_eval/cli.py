...
def audit_verify(
    ...
    from .verification import audit_lifecycle_failures
    ...
    if result.ok and record is not None:
        ...
        try:
            semantic_failures.extend(audit_lifecycle_failures(record, verified_path))
...
def verify_run(
    ...
    from .verification import verify_run as recompute_run_evidence

    report = recompute_run_evidence(run_id)
    ...
    if not report.ok:
        ...
        raise typer.Exit(2)
    console.print(
        f"[green]verified[/green]: {report.subjects_checked} artifact(s), "
