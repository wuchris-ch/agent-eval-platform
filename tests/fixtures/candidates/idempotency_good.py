...
def request(db, value):
    ...
    if amount <= 0:
        return {'status': 422}
    with db:
        old = db.execute('SELECT id,amount FROM orders WHERE tenant=? AND request_key=?', (tenant, key)).fetchone()
        if old:
            return {'status': 201, 'order_id': old[0], 'amount': old[1]} if old[1] == amount else {'status': 409}
